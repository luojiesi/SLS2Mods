...
| Redo after a fast undo ran the enemy turn twice | Runs the game starts itself use its own `NetSingleplayerGameService`; in replay mode the service must report `Replay` so `RequestEnqueue` drops the game's own `ReadyToBeginEnemyTurnAction`. Only runs rebuilt by us had our wrapper service. | (2.2.0) wrap the service for every run; (2.2.1) replaced by a Harmony postfix on `NetSingleplayerGameService.Type`, see next row. |
| Mod failed to load on the beta branch: `Method 'get_LocalVersion' in type 'RewindNetGameService' does not have an implementation` | The beta added a member to `INetGameService`; any class in the mod implementing a game interface makes `Assembly.GetTypes()` throw when the interface changes, which kills the whole mod at load time. | Removed the wrapper class; the `Type` getter of the game's own service is patched instead. Rule: never implement game interfaces in the mod (the self-test's `ICardSelector` is the one remaining case; it only matters for the self-test). |
